...
CrossTable(x=Validation_Data$Flight.Status,y=Predicted_Test_labels, prop.chisq = FALSE)
nb_model <- naiveBayes(Flight.Status~CARRIER+DEST+ORIGIN,data = Validation_Data)
Predicted_Test_labels <-predict(nb_model,Validation_Data, type = "raw")
head(Predicted_Test_labels)
roc(Validation_Data$Flight.Status, Predicted_Test_labels[,2])
plot.roc(Validation_Data$Flight.Status,Predicted_Test_labels[,2])
```
